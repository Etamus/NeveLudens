...
from neveludens.flow_matching_transformer.neveludens import NeveLudens, NeveLudens_Config
from neveludens.mm_tokenizers import NeveLudensTokenizerConfig, NeveLudensTokenizer, Tokenizer
from neveludens.cfg import CkptConfig
...
def normalize_checkpoint_config(raw_config: dict) -> dict:
    config = {**raw_config}
    model_cfg = {**config.get("model_cfg", {})}
    tokenizer_cfg = {**config.get("tokenizer_cfg", {})}
    model_cfg["model_type"] = "neveludens"
    tokenizer_cfg["tokenizer_id"] = "neveludens"
    config["model_cfg"] = model_cfg
    config["tokenizer_cfg"] = tokenizer_cfg
    return config
...
def load_model(checkpoint_path: str):
    ...
    ckpt_config = CkptConfig.model_validate(normalize_checkpoint_config(checkpoint["ckpt_config"]))
    ...
    if isinstance(model_cfg, NeveLudens_Config):
        assert isinstance(tokenizer_cfg, NeveLudensTokenizerConfig), \
            "NeveLudens_Config requires NeveLudensTokenizerConfig for tokenization"
        ...
        tokenizer = NeveLudensTokenizer(tokenizer_cfg)
        game_mapping = tokenizer.game_mapping
        model = NeveLudens(config=model_cfg, game_mapping=game_mapping)
...
class InferenceSession:
    ...
    def __init__(
        ...
        self.is_flowmatching = isinstance(self.ckpt_config.model_cfg, NeveLudens_Config)
